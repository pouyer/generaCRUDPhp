...
class ControladorAcc_rol {
    ...
    public function exportar($formato) {
        try {
            ...
            $filename = "acc_rol_export_{$timestamp}";
            if (!empty($termino)) {
...
$accion = $_GET['action'] ?? '';
$controlador = new ControladorAcc_rol();
...
switch ($accion) {
    ...
    case 'actualizar':
        $id = $_POST['id_rol']; // Usar el campo de llave primaria
        $datos = $_POST;
    ...
    case 'buscar':
        ...
        include '../vistas/vista_acc_rol.php';
        break;
    ...
    default:
        ...
        $dir = $_GET['dir'] ?? 'DESC';
        $offset = ($paginaActual - 1) * $registrosPorPagina;
        $totalRegistros = $controlador->contarRegistrosPorBusqueda(''); // Para obtener total sin filtro
        if (isset($_GET['busqueda'])) {
             // Si hay busqueda pero cae al default
             $termino = $_GET['busqueda'];
             ...
             $totalRegistros = $modeloAcc_rol_count = (new ModeloAcc_rol())->contarRegistros();
        }
        include '../vistas/vista_acc_rol.php';
        break;
}
