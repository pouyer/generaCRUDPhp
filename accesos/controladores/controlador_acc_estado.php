...
switch ($accion) {
    case 'crear':
        ...
        if ($resultado) {
            ...
            $log->registrar($_SESSION['usuario_id'] ?? 0, 'CREAR', 'acc_estado', "Estado creado para tabla: " . ($datos['tabla'] ?? ''));
        }
    ...
    case 'actualizar':
        ...
        if ($resultado) {
            $log = new ModeloAcc_log();
            $log->registrar($_SESSION['usuario_id'] ?? 0, 'ACTUALIZAR', 'acc_estado', "Estado ID $id actualizado");
        }
        echo json_encode($resultado);
        break;
    ...
    case 'eliminar':
        ...
        if ($resultado) {
            $log = new ModeloAcc_log();
            $log->registrar($_SESSION['usuario_id'] ?? 0, 'ELIMINAR', 'acc_estado', "Estado ID $id eliminado");
        }
        echo json_encode($resultado);
        break;
