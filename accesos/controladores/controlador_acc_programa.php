...
switch ($accion) {
    case 'crear':
        ...
        if ($resultado) {
            ...
            $log->registrar($_SESSION['usuario_id'] ?? 0, 'CREAR', 'acc_programa', "Programa creado: " . ($datos['nombre_menu'] ?? ''));
        }
    ...
    case 'actualizar':
        ...
        if ($resultado) {
            $log = new ModeloAcc_log();
            $log->registrar($_SESSION['usuario_id'] ?? 0, 'ACTUALIZAR', 'acc_programa', "Programa ID $id actualizado");
        }
        echo json_encode($resultado);
        break;
    ...
    case 'eliminar':
        ...
        if ($resultado) {
            $log = new ModeloAcc_log();
            $log->registrar($_SESSION['usuario_id'] ?? 0, 'ELIMINAR', 'acc_programa', "Programa ID $id eliminado");
        }
        echo json_encode($resultado);
        break;
