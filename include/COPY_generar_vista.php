...
$contenido .= "        </nav>\n";
    ...
    foreach ($camposValidos as $index => $campo) {
        ...
        //if ($index % 2 == 0) { // Cada dos campos en una fila
        if ($contador % 2 == 1) {    
    ...
    $contenido .= "                                    <input type=\"text\" class=\"form-control\" id=\"{$campos[0]['Field']}\" name=\"{$campos[0]['Field']}\" value=\"<?php echo htmlspecialchars(\$registro['{$campos[0]['Field']}']); ?>\" readonly>\n";
    $contenido .= "                                </div>\n";  
    $contenido .= "                           </div>\n";  
    $contenido .= "                        </div>\n"; 
    $contenido .= "                        <button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\">\n";
    ...
    foreach ($campos as $index => $campo) {
        ...
        //    $contenido .= "                                    <label for=\"{$campo['Field']}\">" . htmlspecialchars($campo['Field']) . ":</label>\n";
            ...
            if ($campo['Key'] == 'PRI') {
                $contenido .= "                                 <input type=\"hidden\" class=\"form-control\" id=\"{$campo['Field']}\" name=\"{$campo['Field']}\" value=\"<?php echo htmlspecialchars(\$registro['{$campo['Field']}']); ?>\" readonly>\n";
            } else 
                {
                    $contenido .= "                                <div class=\"form-group col-md-6\">\n"; // Cambiado a col-md-6
                    $contenido .= "                                    <label for=\"{$campo['Field']}\">" . htmlspecialchars($campo['Field']) . ":</label>\n";
            ...
            $contenido .= "                                </div>\n";
            if ($index % 2 == 1 || $index == count($campos) - 1) { // Cerrar la fila después de dos campos o si es el último
                $contenido .= "                            </div>\n";
            }
